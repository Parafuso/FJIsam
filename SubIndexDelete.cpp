...
//------------------------------------------------------------------------------
//	class	:SubIndex	:サブインデックス(書込関数:レコード削除)
//------------------------------------------------------------------------------
//***** レコード削除
void FJIsam::SubIndex::DeleteRecord( array<Byte>^ inKey, Int64 inRecPos,
							 Int64 inSubIdxPos)
{
	SubIndexRecord^		oldRec;				//確認用レコード
	SubIndexRecord^		tmpOldRec;			//削除前レコードリレーション保存用レコード

	SubIndexRecordPos_T^ tmpDelPos;			//データレコードリレーション

	//-- 引数チェック --//
	if( inKey == nullptr )
	{
		FJIsamException^ tmpFJIsamException= gcnew FJIsamException( L"削除キーが未指定です。" );
		tmpFJIsamException->ErrorCode = 3501;
	...
	if( DelRecLastPos != 0 )
	{
		SubIndexRecord^ tmpDelRec;				//削除レコード
...
//------------------------------------------------------------------------------
//***** レコード削除位置チェック
void FJIsam::SubIndex::ChkDeleteRecordPos( SubIndexRecord^ inRec, SubIndexRecordPos_T^ inDelPos )
{
	SubIndexRecord^			tmpRec;				//作業レコード
	SubIndexRecord^			tmpOldRec;			//作業旧レコード

	SubIndexRecord^			tmpParentRec;		//作業二分木親レコード
	SubIndexRecord^			tmpLtChildRec;		//作業二分木キー小レコード
	SubIndexRecord^			tmpGtChildRec;		//作業二分木キー大レコード

	Int32					tmpLtChildDep;		//二分木小子枝候補階層
	Int32					tmpGtChildDep;		//二分木大子枝候補階層
	SubIndexRecord^			tmpLtChildEndRec;	//二分木キー小側最下層レコード
	SubIndexRecord^			tmpGtChildEndRec;	//二分木キー大側最下層レコード

	//-- 引数チェック --//
	if( inRec == nullptr )
	{
		FJIsamException^ tmpFJIsamException= gcnew FJIsamException( L"データレコードが未設定です。" );
		tmpFJIsamException->ErrorCode = 3502;
		tmpFJIsamException->ErrorCode2 = 1;
		throw tmpFJIsamException;		
	}
	if( inDelPos == nullptr )
